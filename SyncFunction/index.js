...
const spClientId = GetEnvironmentVariable('SpClientId');
const spClientSecret = GetEnvironmentVariable('SpClientSecret');
...
async function addUser(user, _birthday) {
    ...
    try {
        ...
        _context.log(`Èrror adding user ${user.displayName}, error: ${e}`);
    }
...
async function checkUserExist(user) {
    ...
    try {
        ...
        _context.log(`Error Checking if user ${user.displayName} exists, error: ${e} `);
    }
    // Return Value
